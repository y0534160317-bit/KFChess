...
public class Img {
    ...
    // מתודה לעדכון ה-BufferedImage (ליצירת לוח ריק ב-renderer)
    public void createEmpty(int width, int height) {
        this.img = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
    }

    public Img readFromStream(InputStream is) {
        try {
            this.img = ImageIO.read(is);
            if (this.img == null) {
                System.err.println("Error: ImageIO could not decode the stream.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
